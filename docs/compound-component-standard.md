...
├── Sheet.stories.tsx   # витрина: минимум, максимум, крайние случаи
...
2. **Каждая часть опциональна.** Сетка обязана корректно схлопываться, если `Subtitle` или `Close` не передали. Проверяется в stories отдельной историей «минимальный набор».
...
Обязательный минимум историй:

1. **Минимум** — только `Root` + `Body`. Доказывает, что все прочие части опциональны.
2. **Максимум** — все части сразу, все слоты действий.
3. **Крайние случаи** — длинный заголовок без пробелов, длинный скроллящийся контент, состояние загрузки с заблокированным закрытием.

Стори-файл содержит локальную обёртку с триггером, поднимающую `open` в `useState`, — потому что компонент controlled и сам открыться не может.
...
- [ ] Все части в одном файле, рядом с одним CSS-модулем и stories
