...
| `prefix` | string | Префикс пути, добавляется ко всем эндпоинтам (например, `/api`). Пустая строка — если префикса нет. Итоговый URL запроса: `baseUrl` + `prefix` окружения + `prefix` документа + путь эндпоинта |
