...
/// Прокси окружения для исходящего запроса. `None` — идти напрямую: у
/// окружения либо нет настроек, либо прокси выключен.
///
/// Строку здесь не создаём: запрос отправляется куда чаще, чем открываются
/// настройки окружения, и писать в БД ради «прокси нет» незачем.
pub async fn read_config(
    db: &SqlitePool,
    environment_id: &str,
) -> Result<Option<ProxyConfig>, String> {
    Ok(read_by_env_id(db, environment_id)
        .await?
        .and_then(|proxy| proxy.to_config()))
}

pub async fn upsert(db: &SqlitePool, dto: &UpdateEnvironmentProxyDTO) -> Result<(), String> {
    ensure_row(db, &dto.environment_id).await?;
    sqlx::query(
        "UPDATE environment_proxy SET enabled = ?, url = ?, username = ?, password = ?, bypass = ?, insecure = ? WHERE environment_id = ?",
    )
