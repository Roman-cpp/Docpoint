...
impl RequestPayload {
    ...
    /// Подменяет адрес запроса — нужно для query-placement токена: значение
    /// добавляется в строку запроса, а не в заголовки.
...
/// [`cookies`](crate::service::environment::environment_auth::cookies).
pub fn build_client(insecure: bool) -> reqwest::Client {
...
pub struct ClientPool {
    direct_secure: reqwest::Client,
    /// Тот же прямой клиент, но с выключенной проверкой TLS-сертификата.
    /// Раньше `insecure` был подполем прокси и не действовал без него —
    /// самоподписанный сертификат у API без прокси обойти было нечем.
    direct_insecure: reqwest::Client,
...
impl ClientPool {
    pub fn new() -> Self {
        Self {
            direct_secure: build_client(false),
            direct_insecure: build_client(true),
            by_proxy: DashMap::new(),
        }
    }

    /// Клиент для запроса окружения: `proxy = None` — напрямую, `insecure` —
    /// не проверять TLS-сертификат сервера.
    ///
    /// `insecure` учитывается только для прямого пути: при заданном прокси он
    /// уже несёт свой `insecure` внутри [`ProxyConfig`] (см. `to_config`).
    ///
    ...
    pub fn get(
        ...
        let Some(proxy) = proxy else {
            return Ok(if insecure {
                self.direct_insecure.clone()
            } else {
                self.direct_secure.clone()
            });
        };
...
mod tests {
    ...
    async fn every_set_cookie_survives_the_response() {
        ...
        let response = send(&build_client(false), get(&url), None).await.unwrap();
    ...
    async fn a_client_is_built_once_per_proxy() {
        ...
        pool.get(Some(&one), false).unwrap();
        pool.get(Some(&one), false).unwrap();
        pool.get(Some(&two), false).unwrap();
        pool.get(None, false).unwrap();
    ...
    async fn a_broken_proxy_address_is_reported_not_bypassed() {
        ...
        assert!(pool.get(Some(&proxy_at("not a url")), false).is_err());
        ...
        assert!(pool
            .get(Some(&proxy_at("ftp://127.0.0.1:1080")), false)
            .is_err());
        // Без схемы — законный http-прокси.
        assert!(pool.get(Some(&proxy_at("127.0.0.1:8888")), false).is_ok());
        assert!(pool
            .get(Some(&proxy_at("socks5://127.0.0.1:1080")), false)
            .is_ok());
    }

    #[tokio::test]
    async fn direct_path_respects_insecure_without_a_proxy() {
        // Ключевой сценарий бага: без прокси, но с insecure=true — раньше эта
        // комбинация тихо возвращала обычный (проверяющий сертификат) прямой
        // клиент, потому что insecure жил только внутри ProxyConfig, а
        // ClientPool::get(None) не знал о нём вовсе. Полноценная проверка TLS
        // (реальный самоподписанный сертификат) — часть ручной верификации;
        // здесь фиксируем контракт сигнатуры и то, что оба пути рабочие и не
        // требуют прокси.
        let pool = ClientPool::new();
        assert!(pool.get(None, false).is_ok());
        assert!(pool.get(None, true).is_ok());
    }
    ...
    async fn client_does_not_replay_cookies_on_its_own() {
        ...
        send(&client, get(&url), None).await.unwrap();
