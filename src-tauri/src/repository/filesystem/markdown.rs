use crate::domain::markdown::dto::{CreateMarkdownDTO, UpdateMarkdownDTO};
use crate::domain::markdown::entity::{DirListing, FileEntry, FolderEntry, MarkdownFile};
...
/// `vault_dir`: directories are folders and files are documents.
...
impl MarkdownRepository for MarkdownRepo<'_> {
    ...
    async fn create(&self, dto: &CreateMarkdownDTO) -> Result<String, String> {
        ...
        tokio::fs::write(&path, &dto.content)
            .await
            .map_err(|e| e.to_string())?;
