...
/* ─── Helpers ─── */
function hcStatusClass(code: number): string {
...
const HcRequestBlock: FC<{ r: HistoryRecord }> = ({ r }) => {
	const tabs = [
		{ id: "params", lbl: "Параметры", count: r.params.length },
		...
		<div className={s["hc-section"]}>
			...
			{tab === "params" &&
				(r.params.length ? (
					...
					<div className={s["hc-none"]}>Без query-параметров</div>
				))}
...
const HcResponseBlock: FC<{ r: HistoryRecord }> = ({ r }) => {
	const tabs = [
		{ id: "body", lbl: "Тело ответа", count: undefined, disabled: false },
		{
			id: "headers",
			lbl: "Заголовки",
...
/* ─── Drawer (view-only) ─── */
const HcDrawer: FC<{ r: HistoryRecord; onClose: () => void }> = ({
	r,
	onClose,
}) => {
	useEffect(() => {
			...
			>
				<div className={s["hc-drawer-head"]}>
					<div className={s["hc-drawer-head-top"]}>
						<span className={s["hc-drawer-title"]}>{r.title}</span>
						<button
