 ...
 *
 * Файлы из системы бросают в открытый каталог: каждый становится узлом `file`
 * с именем файла.
 */
export const CatalogExplorer: FC<CatalogExplorerProps> = ({
	...
	/** Над окном тащат файлы из системы — открытый каталог готов их принять. */
	...
	// Подписка одна на всё время жизни проводника, а бросить файлы должны в тот
	// каталог, который открыт в момент броска, — поэтому цель читается из ref.
	const dropTargetRef = useRef({ platformId, parentId: catalogId });
	dropTargetRef.current = { platformId, parentId: catalogId };
	const createNodeRef = useRef(createNodeAsync);
	createNodeRef.current = createNodeAsync;

	useEffect(() => {
		let disposed = false;
		let unsubscribe: (() => void) | null = null;

		/** Файлы создаются по одному: имя узла — имя файла, ошибку каждого
		 *  показывает мутация, остальные файлы при этом не теряются. */
		const uploadDropped = async (paths: string[]) => {
			for (const sourcePath of paths) {
				const name = fileNameFromPath(sourcePath);
				if (!name) continue;
				try {
					await createNodeRef.current({
						...dropTargetRef.current,
						name,
						payload: { kind: "file", sourcePath },
					});
				} catch {
					/* тост показывает мутация */
				}
		...
		subscribeFileDropApi((event) => {
			switch (event.type) {
				...
				case "drop":
					setFileDropActive(false);
					void uploadDropped(event.paths);
					break;
			}
		})
			.then((unlisten) => {
				// Эффект успел размонтироваться, пока подписка оформлялась.
			...
			.catch((err) => {
				toast({
					variant: "error",
					title: "Перетаскивание файлов недоступно",
					description: err instanceof Error ? err.message : String(err),
				});
			});
	...
	return (
		<div className={s.explorer}>
			...
			<section className={s.content} onContextMenu={openAreaMenu}>
				...
				<div className={cx(s.listing, fileDropActive && s.listingFileDrop)}>
					{isNodesLoading ? (
