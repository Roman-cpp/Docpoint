...
const exportPlatformCard = async (platform: Platform) => {
	try {
		await exportPlatform(platform.id);
	} catch (err) {
		toast({
			variant: "error",
			title: "Не удалось экспортировать платформу",
			...
			<div className={s.pfCardMenu}>
				<DropMenu>
					...
					<DropMenu.Content>
						...
						<DropMenu.Item onClick={() => exportPlatformCard(platform)}>
							Экспортировать
						</DropMenu.Item>
						<DropMenu.Separator />
...
export const PlatformsSection = () => {
	const { platforms, createPlatform, updatePlatform, isCreating, isUpdating } =
		usePlatformsStore();
	const { importPlatform, isImporting } = useImportPlatform();
	...
	return (
		<div className={s.sbApis}>
			...
			<div className={s.pfActions} style={{ marginTop: 8 }}>
				<button type="button" className={s.sbImportBtn} onClick={openCreate}>
					+ Платформа
				</button>
				<button
					type="button"
					...
					onClick={() => importPlatform()}
				>
					{isImporting ? "Импортируем…" : "Импортировать из zip"}
				</button>
			</div>
