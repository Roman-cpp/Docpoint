...
function toDto(draft: RequestDraft): SaveEndpointRequestDTO {
	return {
		id: draft.id,
		name: draft.name,
...
function save(draft: RequestDraft) {
	return saveEndpointRequestApi(toDto(draft)).catch((e) =>
...
export function useEndpointRequests(endpointId: string) {
	...
	useEffect(() => {
		...
		(async () => {
			try {
				let list = await listEndpointRequestsApi(endpointId);
				if (list.length === 0) {
					list = [await createEndpointRequestApi(endpointId, "Request 1")];
				}
				if (!active) return;
